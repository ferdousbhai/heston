...
- `src/domain/` contains pure schemas, options-temperature calculations, and demo fixtures. It has no framework or network dependency.
...
`wrangler.jsonc` currently binds the existing Reddit Secret Store handles for demo deployment. After creating the tastytrade and Access entries above, add their `secrets_store_secrets` bindings and change `APP_MODE` to `live`. There is intentionally no `.env`, `.env.example`, or `.dev.vars` workflow in this project; local development stays in demo mode and never needs production credentials.
