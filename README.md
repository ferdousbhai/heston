...
For local-first behavior, TanStack DB uses its official local-storage collection for the bounded validated snapshot and preferences, then hydrates an in-memory ticker collection before background revalidation. DXLink events update only that volatile overlay, avoiding synchronous local-storage writes on every tick; the app asks the browser for persistent storage and refreshes stale snapshots when it reconnects or returns to the foreground.
