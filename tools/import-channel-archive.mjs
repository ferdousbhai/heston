...
const ENTITIES = { '&#39;': "'", '&amp;': '&', '&gt;': '>', '&lt;': '<', '&nbsp;': ' ', '&quot;': '"' }
const unescape = (s) => s.replace(/&#(\d+);/g, (_, code) => String.fromCodePoint(Number(code))).replace(/&(?:#39|amp|gt|lt|nbsp|quot);/g, (m) => ENTITIES[m])
...
for (const { html, id, postedAt } of rows) {
  ...
  const text = unescape(html
    .replace(/<br\s*\/?>/gi, '\n')
    .replace(/<a [^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi, (_, href, label) => (label.replace(/<[^>]+>/g, '').trim() === href.trim() ? href : `${label} (${href})`))
    .replace(/<[^>]+>/g, ''))
    .replace(/[ \t]+\n/g, '\n').trim()
  if (!text || text === NOTICE) continue
  const q = (s) => `'${String(s).replace(/'/g, "''")}'`
  statements.push(`INSERT OR REPLACE INTO long_vol_channel_posts (post_id, posted_at, text, links_json, imported_at) VALUES (${id}, ${q(new Date(postedAt).toISOString())}, ${q(text)}, ${q(JSON.stringify(links))}, ${q(new Date().toISOString())});`)
}
