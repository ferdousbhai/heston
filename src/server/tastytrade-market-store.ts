...
async function runBatches(db: D1Database, statements: readonly D1PreparedStatement[]): Promise<void> {
...
export async function persistTastytradeMarketSnapshot(
  ...
  const statements = tickers.flatMap((ticker) => {
    const term = ticker.ivTermStructure
    const previousClose = ticker.price - ticker.change
    return [
      env.DB!.prepare(
        ...
        ticker.symbol, ticker.ivIndex, ticker.ivRank, ticker.ivPercentile,
      ...
      env.DB!.prepare(
        ...
        ticker.symbol, ticker.price, previousClose, ticker.change, ticker.changePercent,
