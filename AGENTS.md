...
Spice is a single-owner options app on Cloudflare. Public visitors may read a neutral Options Watch, option metrics, catalysts, and the Daily Brief. Only the exact Google-authenticated owner may access positions, balances, transactions, source watchlists, Dan, operations, live account streams, or any mutation.
...
| Internal watchlist, instrument catalog, one-time broker seed, public universe | `src/server/internal-watchlist.ts`, `src/server/instrument-catalog.ts`, `src/server/public-market-universe.ts`, `src/server/tastytrade.ts`, `migrations/0006_internal_watchlist.sql`, `migrations/0008_instrument_catalog.sql`, `migrations/0009_instrument_catalog_resolution.sql`, `ops/`, `tools/seed-internal-watchlist.sh` |
| Source-specific market storage and display views | `src/server/tastytrade-market-store.ts`, `src/server/catalysts.ts`, `migrations/0010_source_specific_market_data.sql` |
...
| Catalyst contract and D1 persistence | `src/domain/catalyst.ts`, `src/server/catalysts.ts`, `migrations/` |
...
- Spice's bounded D1 internal watchlist is authoritative. Its explicit one-time bootstrap preserves every tastytrade private and public source row and entry privately, but only the selected 100 live rows remain in `internal_watchlist_items`; the instrument catalog retains the wider reference universe. After `ready`, normal code must never read or mutate tastytrade watchlist endpoints.
- The maintained internal watchlist is capped at 100 symbols. It prioritizes active positions, owner additions, other deterministically validated Spice additions, former private-list members, then eligible individual equities by the retained one-time High Options Volume order. Adding at capacity evicts the lowest-priority retained seed member; direct deletion requires an explicit owner request. Only the source-neutral, alphabetized union is stored for public reads. No public field or ordering reveals that priority or ticker provenance.
...
- D1 stores a typed projection of every interesting tastytrade Equity field, never raw instrument JSON. Stable identity is used by research and public rendering; trading availability refreshes daily and execution-critical restrictions still refresh in the order path. Yahoo data is bounded, delayed secondary research context only.
