...
the `daily_research` prompt this Worker serves, which is the whole recipe; no process on this
Worker produces one.
...
- **Agent loops do not run on Cloudflare.** The Worker serves a stateless MCP surface gated by
  ...
  commands. **The public brief is generated by members' own agents, never by this Worker**: any
  signed-in member's agent may publish one, and what it submits is untrusted
