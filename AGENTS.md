...
| Catalyst contract, manual Codex bootstrap, and D1 persistence | `src/domain/catalyst.ts`, `src/server/catalysts.ts`, `src/server/catalyst-bootstrap.ts`, `ops/catalyst-research/`, `migrations/` |
...
- Manual Codex catalyst findings are always `estimated`. A source URL is accepted only when the Codex transcript records an exact direct page open; its display label is derived from the verified hostname, never model-authored source metadata.
