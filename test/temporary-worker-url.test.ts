...
describe('temporary Worker URL extraction', () => {
  it('selects the workers.dev route for the exact uniquely named Worker', () => {
    const output = `
      ...
      Deployed heston-catalog-20260829T120000Z-a1b2c3d4
      https://heston-catalog-20260829T120000Z-a1b2c3d4.owner.workers.dev
    `
    expect(extractTemporaryWorkerUrl(output, 'heston-catalog-20260829T120000Z-a1b2c3d4')).toBe(
      'https://heston-catalog-20260829t120000z-a1b2c3d4.owner.workers.dev',
    )
  })
  ...
  it('rejects a route belonging to another deployment', () => {
    expect(() => extractTemporaryWorkerUrl(
      'https://heston-catalog-old.owner.workers.dev',
      'heston-catalog-new',
    )).toThrow('Wrangler did not report the workers.dev URL for heston-catalog-new.')
  })
})
