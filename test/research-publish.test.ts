...
/** What the Worker's browser returns for every page this run reads. */
function publishingEnv(markdown = PAGE_MARKDOWN) {
  return { BROWSER: markdownBrowser(markdown), DB: store.database }
}

/** The stored brief, parsed through the same contract the site reads it with. */
function storedBrief(id: string): DailyRecommendations {
...
describe('publishing a submission produced off this Worker', () => {
  it('re-reads the cited page, binds everything, and persists', async () => {
    const publication = await publishSubmittedDailyRecommendations(
      publishingEnv(),
      submission(),
      { now: NOW, publishedByUserId: PUBLISHER },
    )
  ...
  it('publishes the chosen byline and the quotes, and keeps the publisher private', async () => {
    await expect(publishSubmittedDailyRecommendations(
      publishingEnv(),
      { ...submission(), byline: 'volhound' },
      { now: NOW, publishedByUserId: PUBLISHER },
    )).resolves.toMatchObject({ status: 'published' })

    const brief = storedBrief('recommendations-2026-09-02')
  ...
  it('lets a brief stand for the refresh interval before another may replace it', async () => {
    await expect(publishSubmittedDailyRecommendations(publishingEnv(), submission(), { now: NOW, publishedByUserId: PUBLISHER }))
      .resolves.toMatchObject({ status: 'published' })
