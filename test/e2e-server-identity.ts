 ...
 * happened to hold port 3000, and every failure read as a bug in spicy.trade.
 *
 ...
 * server, while an answer that is not spicy.trade's -- including HTML from an app whose SPA fallback
...
export default async function assertServerIsSpice(): Promise<void> {
  ...
  // A server that answers but cannot say what it is will not be reused as spicy.trade either, so this
  ...
  throw new Error(
    `${baseUrl} is already serving something that is not spicy.trade (${seen}). Playwright would reuse `
