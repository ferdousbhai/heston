 ...
 * happened to hold port 3000, and every failure read as a bug in Spice.
 *
 ...
 * server, while an answer that is not Spice's -- including HTML from an app whose SPA fallback
...
export default async function assertServerIsSpice(): Promise<void> {
  ...
  // A server that answers but cannot say what it is will not be reused as Spice either, so this
  ...
  throw new Error(
    `${baseUrl} is already serving something that is not Spice (${seen}). Playwright would reuse `
